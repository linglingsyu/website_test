...
<body>
        ...
        <div id="main">
                ...
                <div id="left" class="ct">
                        ...
                        <a href="?do=main" style="color:#f00;">全部商品(<?=$Goods->count(['sh'=>1])?>)</a>
                        <?php
                        $rows = $Type->all(['parent'=>0]);
                        foreach($rows as $row){
                        ?>
                        <div class="ww"><a href="?do=main&type=<?=$row['id']?>" style="color:#f00;"><?=$row['name']?>(<?=$Goods->count(['sh'=>1,'big'=>$row['id']])?>)</a>
                                <?php
                                $subs = $Type->all(['parent'=>$row['id']]);
                                foreach($subs as $s){        
                                ?>
                                 <div class="s" >
                                 <a href="?do=main&type=<?=$s['id']?>" style="color:#fa0;"><?=$s['name']?>(<?=$Goods->count(['sh'=>1,'mid'=>$s['id']])?>)</a>
                                 </div>
                                <?php
                                }
                                ?>
